...
export default function GalleryPage() {
  ...
  const filteredArchive = selectedScale ? archiveCars.filter(car => car.scale === selectedScale) : archiveCars;
      ...
      <section className="mb-32">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 auto-rows-[400px]">
          {featuredCars.map((car) => (
            <BentoCard key={car.id} car={car} setHoverState={setHoverState} setCursorBlocked={setCursorBlocked} />
          ))}
        </div>
      </section>
      ...
      <section className="mb-20">
         ...
         <div className="flex flex-wrap justify-between items-end gap-6 mb-12">
           ...
           <div className="flex gap-2">
              <button onClick={() => setSelectedScale(null)} className={`px-4 py-1.5 text-[10px] font-mono rounded-full border transition-all uppercase tracking-widest ${!selectedScale ? 'bg-black text-white border-black' : 'bg-transparent text-gray-400 border-gray-200 hover:border-black'}`}>All</button>
              {uniqueScales.map(scale => (
                <button key={scale} onClick={() => setSelectedScale(scale)} className={`px-4 py-1.5 text-[10px] font-mono rounded-full border transition-all uppercase tracking-widest ${selectedScale === scale ? 'bg-black text-white border-black' : 'bg-transparent text-gray-400 border-gray-200 hover:border-black'}`}>{scale}</button>
              ))}
           </div>
        </div>
...
function BentoCard({ car, setHoverState, setCursorBlocked }) {
  ...
  return (
    <motion.div
      onClick={() => router.push(`/product/${car.id}`)} // NAVIGATE ON CLICK
      onMouseEnter={() => { setIsHovered(true); setHoverState(true); }}
      onMouseLeave={() => { setIsHovered(false); setHoverState(false); }}
      className="relative md:col-span-2 md:row-span-1 rounded-sm overflow-hidden bg-white border border-gray-100 group shadow-sm hover:shadow-2xl transition-all duration-700 cursor-pointer"
    >
      <div className="absolute inset-0 flex items-center justify-center bg-[#fdfdfd] p-12">
        <AnimatePresence mode="wait">
           {!isHovered ? (
             <motion.img 
               layoutId={`car-image-${car.id}`} // SHARED LAYOUT ID
               key="img" src={car.image} className="w-full h-full object-contain drop-shadow-xl"
  ...
  return (
    ...
    >
      <div className="relative aspect-square bg-[#f8f8f8] rounded-sm flex items-center justify-center p-6 mb-3 border border-transparent group-hover:border-black/5 transition-colors">
        <motion.img 
          layoutId={`car-image-${car.id}`} // SHARED LAYOUT ID
          src={car.image} className="w-full h-full object-contain grayscale group-hover:grayscale-0 transition-all duration-500 group-hover:scale-110" 
        />
        <div className="absolute top-2 left-2">
          <span className="text-[8px] font-mono border border-black/10 bg-white px-1.5 py-0.5 rounded text-gray-500">{car.scale}</span>
        </div>
...
// ... (Keep CustomCursor same as before) ...
