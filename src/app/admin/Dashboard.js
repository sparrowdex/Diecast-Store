...
export default function AdminDashboard({ initialCars = [], initialOrders = [], dbError = false }) {
  ...
  // Stats Calculation
  const totalValue = cars.reduce((acc, car) => acc + parseInt(String(car.price).replace(/[^\d]/g, "") || 0), 0);
  ...
  const podium = [...cars].sort((a, b) => 
    parseInt(String(b.price).replace(/[^\d]/g, "") || 0) - parseInt(String(a.price).replace(/[^\d]/g, "") || 0)
  ).slice(0, 3);
  ...
  return (
    <div className="p-4 md:p-8 lg:p-12 max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-6 mb-12">
        <div>
           <h2 className="text-2xl md:text-3xl font-black uppercase italic tracking-tighter mb-2">Pit_Wall_Telemetry</h2>
