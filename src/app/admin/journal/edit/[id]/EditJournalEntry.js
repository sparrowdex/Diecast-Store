...
export default function EditJournalEntry({ entry }) {
  ...
  const [formData, setFormData] = useState({ 
    ...
    genre: entry.genre || "CLASSIC_VINTAGE",
    isPublished: entry.isPublished ?? false
  });

  const handleUpdate = async (e) => {
    e.preventDefault();
    // Destructure to remove non-schema fields and internal UI state before sending to API
    const { imageUrl, videoUrl, id, createdAt, updatedAt, images, video, ...rest } = formData;
    ...
    if (response.ok) {
      // Re-route to Preview mode after updating
      router.push(`/admin/journal/preview/${entry.id}`);
    }
  };
    ...
    <div className="p-12 text-white font-geist max-w-6xl mx-auto">
      ...
      <form onSubmit={handleUpdate} className="space-y-12">
        ...
        <div className="flex justify-end gap-6 pt-10 border-t border-white/10">
          <button type="submit" className="bg-white text-black px-10 py-3 font-black text-[10px] uppercase italic hover:bg-[#FF8700] hover:text-white">
            Commit_Changes_&_Preview
          </button>
        </div>
      </form>
    </div>
